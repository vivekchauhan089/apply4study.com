...
export default function ChatBox() {
  ...
  const handleSendMessage = () => {
    let obj = {
      ...
      sender: selectedUserForChat.latestMessage.sender
    };
  ...
  useEffect(() => {
    return () => {
      ...
      webSocket.onopen("message received");
    };
  }, [webSocket]);
  ...
  useEffect(() => {
    if (!sendMessageProcessing && !sendMessageFail && sendMessageSuccess) {
      ...
      webSocket.onopen("new message", sendMessageObj);
  ...
  useEffect(() => {
    ...
    webSocket.onmessage("message received", handleNewMessageReceived);
